...
namespace ricetta_dematerializzata_dll.Core
{
    ...
    internal class SoapHttpClient
    {
        ...
        public string ChiamaServizio(string url, string soapAction, string soapEnvelope, string? authorization2F = null)
        {
            ...
            else if (_ambiente == ServiceEnvironment.Produzione &&
            ...
            // Certificato client SSL solo in produzione
            if (_ambiente == ServiceEnvironment.Produzione &&
                !string.IsNullOrEmpty(_pathCertificatoSsl) && File.Exists(_pathCertificatoSsl))
                req.ClientCertificates.Add(new X509Certificate2(_pathCertificatoSsl));
