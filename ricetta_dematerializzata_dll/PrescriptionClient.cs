...
using ricetta_dematerializzata_dll.Crypto;
...
namespace ricetta_dematerializzata_dll.Services
{
    ...
    ///   var client = new SanitaServiceClient(cfg);
    ...
    public class PrescriptionClient : ISanitaServiceClient
    {
        ...
        public PrescriptionClient(ServiceConfiguration configurazione)
        {
        ...
        // ── ISanitaServiceClient (interfaccia COM-friendly) ───────────────────────
        ...
        /// <param name="ignoraSsl">true = ignora errori SSL (solo sviluppo)</param>
        public void Configura(string username, string password,
        ...
        /// <summary>
        /// Configura i path dei certificati SSL e Sanitel.
        /// </summary>
        public void ConfiguraCertificati(
            string? pathSsl, string? pathCA, string? pathSanitel)
        {
            if (_config == null)
                throw new InvalidOperationException("Chiamare prima Configura().");

            _config.PathCertificatoSsl     = pathSsl;
            _config.PathCertificatoCA      = pathCA;
            _config.PathCertificatoSanitel = pathSanitel;
        ...
        /// </summary>
        /// <param name="servizio">Servizio da chiamare (valore intero dell'enum ServizioSanita)</param>
        ...
        /// <summary>
        /// Cifra automaticamente CODICE_FISCALE_ASSISTITO e PINCODE se:
        ...
        private void CifraParametriSensibili(Dictionary<string, string> dict)
        {
            ...
            var pathSanitel = OttieniPathCertificatoSanitel();
            var campiDaCifrare = new[]
            {
                "CODICE_FISCALE_ASSISTITO", "CF_ASSISTITO", "cfAssistito",
                "PINCODE", "pinCode"
            };
        ...
        private bool ContieneCampiSensibili(Dictionary<string, string> dict)
            => dict.ContainsKey("CODICE_FISCALE_ASSISTITO")
               ...
               || dict.ContainsKey("cfAssistito")
               || dict.ContainsKey("PINCODE")
               || dict.ContainsKey("pinCode");

        private string OttieniPathCertificatoSanitel()
        {
            var pathConfigurato = _config?.PathCertificatoSanitel;
            if (!string.IsNullOrWhiteSpace(pathConfigurato) && File.Exists(pathConfigurato))
                return pathConfigurato;

            var pathDefault = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "certificates", NomeCertificatoSanitelDefault);
            if (File.Exists(pathDefault))
                return pathDefault;

            throw new InvalidOperationException(
                $"Certificato Sanitel richiesto per cifrare pinCode/CF assistito non trovato. " +
                $"Impostare PathCertificatoSanitel o distribuire '{NomeCertificatoSanitelDefault}' in 'certificates'.");
        }
